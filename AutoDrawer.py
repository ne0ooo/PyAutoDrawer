...
group.add_argument('-v', '--verbose', action='store_true', help='Print out every single point to be clicked / dragged through')
        ...
        for x in range(image.size[1]):
            if args.inverted:
                if image_pixels[x, y][3] != 0:
                    ...
                    if args.verbose:
                        print(f'Up x:{x=x+starting_position[0]}, y:{ y=y+starting_position[1]}')
                else:
                    pyautogui.mouseDown(x=x+starting_position[0], y=y+starting_position[1])
                    if args.verbose:
                        print(f'Down x:{x=x+starting_position[0]}, y:{ y=y+starting_position[1]}')
            else:
                if image_pixels[x, y][3] != 0:
                    pyautogui.mouseDown(x=x+starting_position[0], y=y+starting_position[1])
                    if args.verbose:
        ...
        for x in range(image.size[1]):
            if args.inverted:
                if image_pixels[x, y][3] == 0:
                    ...
                    if args.verbose:
                        print(f'Click x:{x=x+starting_position[0]}, y:{ y=y+starting_position[1]}')
